    ...
    <div class="layui-container">
        <ul class="layui-clear">
            ...
            <#list categories as item>
                <li class="${(currentCategoryId == item.id) ? string('layui-hide-xs layui-this', '')}"><a
                            href="/category/${item.id}">${item.name}</a></li>
            </#list>
            <li class="layui-hide-xs layui-hide-sm layui-show-md-inline-block"><span class="fly-mid"></span>
            </li>
            <@shiro.user>
                <!-- 用户登入后显示 -->
